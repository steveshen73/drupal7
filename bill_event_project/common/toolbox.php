...
function dbConnect() {
	if (list(,$lcc,$dbc) = file('sites/all/themes/clouds/bill_event/lcaldbc.dat',FILE_IGNORE_NEW_LINES)) { //get version + db creds
		define("LCC",$lcc);
...
function sendMail($subject,$message,$emlList,$senderId=0) {
	...
	foreach ($notList as $emlAorL) { //create email address list
		if (strpos($emlAorL,'@')) { //email address
			...
			if (file_exists("sites/all/themes/clouds/bill_event/emlists/$emlAorL")) {
				$addressList = array_merge($addressList,file("sites/all/themes/clouds/bill_event/emlists/$emlAorL", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES));
			}
		}
	}
